...
import { inventoryService, InventoryItem, Recipe } from '@/lib/inventory.service';
...
export default function EditRecipePage() {
  ...
  const loadData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [fetchedRecipe, menuData, invData] = await Promise.all([
        inventoryService.getRecipe(id),
      ...
      reset({
        ...
        menuItemId: fetchedRecipe.menuItemId ?? '',
      });
      setIngredients(
  ...
  }, [id, user?.restaurantId, reset, router]);
  ...
  const onSubmit = handleSubmit(async (values) => {
    ...
    setSubmitting(true);
    try {
      await inventoryService.updateRecipe(id, {
        name: values.name,
    ...
    <div className="mx-auto max-w-3xl space-y-6">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-semibold text-slate-950">
          {recipe?.name ?? 'Edit Recipe'}
        </h1>
        <p className="mt-2 text-sm text-slate-600">
          Update the recipe name, linked menu item, or ingredient quantities.
        </p>
      </div>
